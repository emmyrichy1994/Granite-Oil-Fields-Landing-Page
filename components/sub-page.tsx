...
export function SubPage({ page }: { page: string }) {
  ...
  return <main><Navigation/><section className="subhero"><p className="eyebrow">GRANITE OILFIELD SERVICES</p><h1>{isServices ? 'Our Services' : copy.title}</h1><p>{isServices ? 'Operational support for drilling, completions, frac monitoring and well plugging.' : copy.intro}</p></section>{isServices ? <section className="serviceDetails">{services.map((service,index)=><article id={service.id} key={service.id}><div className="serviceDetailVisual"><span>0{index+1}</span><h2>{service.title}</h2></div><div className="serviceDetailCopy"><p className="eyebrow">GRANITE OILFIELD SERVICES</p><h2>{service.title} support for well-planned operations.</h2><p>{service.summary}</p><ul>{service.features.map(feature=><li key={feature}><Check size={18}/>{feature}</li>)}</ul><Link href="/contact" className="textLink">Talk with our team <ArrowRight size={17}/></Link></div></article>)}</section> : <section className="pageContent"><p className="eyebrow">{copy.label}</p><h2>{copy.heading}</h2><p>{copy.body}</p><Link href="/contact" className="button dark">Contact Our Team</Link></section>}</main>;
}
